...
const GLOBAL_CHART = gql`
  query solidlyDayDatas($startTime: Int!, $skip: Int!) {
    solidlyDayDatas(
      first: 1000
      skip: $skip
      subgraphError: allow
...
async function fetchChartData(client: ApolloClient<NormalizedCacheObject>) {
  ...
  try {
    while (!allFound) {
      ...
      if (!loading) {
        skip += 1000
        if (chartResData.solidlyDayDatas.length < 1000 || error) {
          allFound = true
        }
        if (chartResData) {
          data = data.concat(chartResData.solidlyDayDatas)
        }
      }
    }
  } catch {
...
export function useFetchGlobalChartData(): {
  ...
  const shouldUserDerivedData = activeNetworkVersion.id === SupportedNetwork.ETHEREUM
